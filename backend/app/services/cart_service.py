...
from ..schemas.cart import CartItemCreate, CartItemUpdate, CartResponce, CartItem
...
class CartService:
    ...
    def add_to_card(self, cart_data: Dict[int, int], item: CartItemCreate) -> Dict[int, int]:
        ...
        if item.product_id in cart_data:
            cart_data[item.product_id] += 1
        else:
            cart_data[item.product_id] = item.quantity

        return cart_data
    
    def update_card(self, cart_data: Dict[int, int], item: CartItemCreate) -> Dict[int, int]:
    ...
    def remove_from_cart(self, cart_data: Dict[int, int], item: CartItemCreate) -> Dict[int, int]:
        if item.product_id not in cart_data:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Product with id - {item.product_id} not found in cart")
        del cart_data[item.product_id]
        return cart_data
    
    def get_cart_detail(self, cart_data: Dict[int, int]) -> CartResponce:
        if not cart_data:
        ...
        return CartResponce(items=cart_items, total=round(total_price, 2),
