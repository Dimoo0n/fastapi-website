...
from ..schemas.product import ProductResponce, ProductCreate, ProductListResponce
...
class ProductService:
    ...
    def get_all_product(self) -> ProductListResponce:
    ...
    def get_products_by_category(self, category_id: int) -> ProductListResponce:
        ...
        if not category:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Category with id - f{category_id} not found")
    ...
    def create_product(self, product_data: ProductCreate) -> ProductResponce:
        ...
        if not category:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Category with id - f{product_data.category_id} not found")
